<template>
  <div class="ratingselect">
    <div class="rating-type border-1px">
      <span class="block" :class="{active: selectRateType===2}" @click="setSelectRateType(2)">
        全部<span class="count">{{ratingsTotalCount}}</span>
      </span>
      <span class="block" :class="{active: selectRateType===0}" @click="setSelectRateType(0)">
        推荐<span class="count">{{ratingsPositiveCount}}</span>
      </span>
      <span class="block" :class="{active: selectRateType===1}" @click="setSelectRateType(1)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //  通过vue分发自定义事件，因为没有这个状态，父组件有
        this.$emit('setSelectRateType', selectRateType)
      },
      switchOnlyContent() {
