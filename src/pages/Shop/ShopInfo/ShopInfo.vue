  ...
  export default {
    ...
    methods: {
      _initScroll () {
        ...
        // 在这之前，需要动态计算ul元素的宽度，根据请求的图片个数
        const ul = this.$refs.picUl
        const liWidth = 120  // 图片的宽度
        const space = 6  // li之间的间隔
        const size = this.info.pics.length  // 请求的图片url个数
        ul.style.width = (liWidth + space) * size - space + 'px'  // 最后一个不需要留空白间隔
